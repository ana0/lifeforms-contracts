 ...
 */
contract Lifeforms2 is Context, ERC165, IERC721, IERC721Metadata, Ownable, ContextMixin, NativeMetaTransaction {
    ...
    mapping (uint256 => uint) private _tokenBirth;

    // Mapping from token ID to duration of this ownership period
    mapping (uint256 => uint) private _tokenOwnerBeginning;

    // Mapping from token ID to blockhash at birth
    mapping (uint256 => bytes32) public _seed;
     ...
     */
    constructor (string memory name_, string memory symbol_, uint256 _maxDuration, uint256 price_) Ownable() {
        _name = name_;
        _symbol = symbol_;
    ...
    function isDead(uint256 tokenId) public view returns (bool) {
        ...
        if (block.timestamp - _tokenOwnerBeginning[tokenId] > maxDuration) {
            return true;
        }
    ...
    function _birth(uint256 tokenId) internal {
        ...
        _tokenOwnerBeginning[tokenId] = block.timestamp;
        _seed[tokenId] = blockhash(block.number-1);
    ...
    function ownerBirth(address to, uint256 tokenId) public onlyOwner {
        _mint(to, tokenId);
        _birth(tokenId);
    }
    ...
    */
    ...
    function tokenOwnerBeginning(uint256 tokenId) public view virtual returns (uint256) {
        if (isAlive(tokenId)) {
            return _tokenOwnerBeginning[tokenId];
        } else {
            return 0;
        }
    }

    function tokenBirth(uint256 tokenId) public view virtual returns (uint256) {
        if (isAlive(tokenId)) {
            return _tokenBirth[tokenId];
        } else {
            return 0;
        }
    }
    ...
    function getLifeform(uint256 tokenId) public view virtual returns (string memory, address, uint256, uint256, bytes32) {
        if (isAlive(tokenId)) {
            return (tokenURI(tokenId), ownerOf(tokenId), _tokenBirth[tokenId], _tokenOwnerBeginning[tokenId], _seed[tokenId]);
        } else {
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = Lifeforms2.ownerOf(tokenId);
        require(to != owner, "ERC721: approval to current owner");

        require(_msgSender() == owner || Lifeforms2.isApprovedForAll(owner, _msgSender()),
    ...
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal view virtual returns (bool) {
        ...
        address owner = Lifeforms2.ownerOf(tokenId);
        return (spender == owner || getApproved(tokenId) == spender || Lifeforms2.isApprovedForAll(owner, spender));
    }
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
        require(Lifeforms2.ownerOf(tokenId) == from, "ERC721: transfer of token that is not own"); // internal owner
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(Lifeforms2.ownerOf(tokenId), to, tokenId); // internal owner
    }
